...
namespace CustomPlayerController
{
    public class CustomCameraController : MonoBehaviour
    {
		...
		protected CustomCameraInput cameraInput;

		//Setup references.
		void Awake () {
			tr = transform;
			cam = GetComponent<Camera>();
			cameraInput = GetComponent<CustomCameraInput>();
		...
		public void RotateTowardDirection(Vector3 _direction, float _lookSpeed)
		{
			...
			float _xAngleDifference = CustomVectorMath.GetAngle(new Vector3(0f, _currentLookVector.y, 1f), new Vector3(0f, _direction.y, 1f), Vector3.right);

			//Calculate y angle difference;
			_currentLookVector.y = 0f;
			_direction.y = 0f;
			float _yAngleDifference = CustomVectorMath.GetAngle(_currentLookVector, _direction, Vector3.up);
