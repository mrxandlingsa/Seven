...
namespace CustomPlayerController
{
    public class CustomMover:MonoBehaviour
    {
		...
		[SerializeField] public CustomSensor.CastType sensorType = CustomSensor.CastType.Raycast;
		...
		void Awake()
		{
			...
			//Initialize sensor;
			sensor = new CustomSensor(this.tr, col);
		...
		void OnValidate()
		{
			...
			if(sensorType == CustomSensor.CastType.RaycastArray)
				raycastArrayPreviewPositions = 
					CustomSensor.GetRaycastStartPositions(sensorArrayRows, sensorArrayRayCount, sensorArrayRowsAreOffset, 1f);
		}
		...
		void RecalibrateSensor()
		{
			...
			sensor.SetCastDirection(CustomSensor.CastDirection.Down);
